...
(async function main() {
  ...
  ok('bank 为数组且 502 题', Array.isArray(bank) && bank.length === 502, 'length=' + (bank && bank.length));
  ...
  ok('题型分布 单选374/多选106/案例22', singles === 374 && multis === 106 && cases === 22, `single=${singles} multi=${multis} case=${cases}`);
  ...
  const t = w.__t;
  ok('BANK 注入 502', t.BANK.length === 502, 'BANK.length=' + t.BANK.length);
  ...
  ok('三轮累计覆盖全部 502 题', seen.size === 502, 'covered=' + seen.size);
