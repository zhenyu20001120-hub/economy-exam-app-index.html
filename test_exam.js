...
const r2 = w.eval('state').items.map(q => q.id);
ok('第2轮题数≥150', r2.length >= 150);
ok('第1轮与第2轮无重复(跨轮去重)', r1.filter(id => r2.includes(id)).length === 0);

// 第3轮: 覆盖剩余, 三轮并集=全库502
w.eval("startMode('round',3)");
const r3 = w.eval('state').items.map(q => q.id);
ok('第3轮题数≥150', r3.length >= 150);
